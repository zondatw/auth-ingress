...
def test_admin_user_list_and_detail_contract(client, csrf, db):
    ...
    detail = client.get(f"/admin/users/{db.query(__import__('auth_entry_portal.models', fromlist=['User']).User).filter_by(email='member@example.test').one().id}")
...
def test_membership_preview_does_not_write(client, csrf, db_factory, db):
    sign_in(client, csrf, email="admin@example.test")
    user = db.query(__import__("auth_entry_portal.models", fromlist=["User"]).User).filter_by(email="outsider@example.test").one()
    group = db.query(__import__("auth_entry_portal.models", fromlist=["Group"]).Group).filter_by(name="staff").one()
    ...
    with db_factory() as check:
        assert check.query(__import__("auth_entry_portal.models", fromlist=["GroupMembership"]).GroupMembership).filter_by(user_id=user.id).count() == 0
