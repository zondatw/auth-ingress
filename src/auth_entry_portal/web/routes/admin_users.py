...
from auth_entry_portal.models import Group
...
def reset_password(user_id: int, request: Request, expected_revision: int = Form(...), confirm: bool = Form(False), csrf: str = Form(...), identity: Identity = Depends(require_admin), db: Session = Depends(get_db), settings: Settings = Depends(get_settings)):
    if not valid_csrf(csrf, settings): return detail_page(request, db, settings, identity, user_id, error="The form expired. Please try again.", status_code=400)
    try:
        actor = __import__("auth_entry_portal.services.user_admin_service", fromlist=["require_admin_actor"]).require_admin_actor(db, identity.user)
        target = db.get(__import__("auth_entry_portal.models", fromlist=["User"]).User, user_id)
        ...
        if not confirm:
            result = __import__("auth_entry_portal.services.user_management_types", fromlist=["OperationResult"]).OperationResult("password_reset", OutcomeCode.SUCCESS, target.id, target.revision, message="Preview password reset")
